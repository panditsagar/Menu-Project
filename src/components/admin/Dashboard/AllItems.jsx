...
const AllItems = () => {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get(`${BASE_URL}items/get/${categoryId}`, {
  ...
  const handleDelete = async (itemId) => {
    try {
      const res = await axios.delete(`${BASE_URL}items/delete/${itemId}`, {
        withCredentials: true,
      });
      if (res.data.success) {
        fetchItems();
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex justify-center ">
      <div className="w-full max-w-5xl space-y-1">
        ...
        <div className="flex items-center  gap-2 bg-white shadow-md px-6 pb-6 md:px-6 md:pb-6">
          <Link to={`/admin/dashboard`} className="pt-3">
        ...
        <div className="bg-white p-4 md:p-6 shadow-md overflow-x-auto">
          <h1 className="text-xl pb-3 text-gray-400">
            All {item?.[0]?.categoryId?.name} items
          </h1>
          <table className="w-full min-w-[600px]">
            <thead className="bg-gray-200">
              <tr>
                ...
                <th className="py-3 px-4 font-medium text-gray-600  ">Edit</th>
            ...
            <tbody>
              {item?.length === 0 ? (
                ...
                item?.map((item, index) => (
                  <tr
                    key={index}
                    className="hover:bg-gray-50 transition border-b border-gray-300 last:border-0 cursor-pointer"
                  >
                    ...
                    <td className="py-3 px-4 text-gray-500 text-center">
                      <EditIcon
                        onClick={() =>
                          navigate(`/admin/edititem/${item._id}/${categoryId}`)
                        }
                      />
                    </td>
                    <td className="py-3 px-4 text-gray-500 text-center">
